...
y_p = np.array(y_p)
y_axis = np.linspace(0, 1, len(y_p))
normalized = (y_p-min(y_p))/(max(y_p)-min(y_p))
maxDistance = 0
indexMax = 0
...
#plt.scatter(np.array([int(indexMax)]), np.array([normalized[indexMax]]), c='black', label='Estimated 70% recall point', s = 40)
plt.scatter(np.array([int(indexMax)]), np.array([normalized[indexMax]]), c='black', label='Max Distance', s = 40)
#plt.title('CD010023')
#plt.scatter(np.array([int(x_sam_val)]), np.array([y_p[x_sam_val]]), c='black', label='Estimated 70% recall point', s = 40)
plt.plot(range(0, len(y_p)), normalized)
plt.plot(range(0, len(y_p)), y_axis)
